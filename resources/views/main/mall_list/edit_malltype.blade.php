        ...
        <div class="col-md-10">
            <div class="card card-malle">
                <div class="card-header-malle">
                   <p>Type Id: <span style="margin-right: 120px;color: red">{{ $malltype->mt_id }}</span><span style="float: right"><a href="{{ route('mall-type') }}">Back</a></span></p>
                </div>
                <div class="card-body" id="tag-image-body" data-sourceurl="{{route('mall-type.edit',[$malltype->mt_id])}}">
                    <form method="PATCH" action="{{route('mall-type.update',[$malltype->mt_id])}}" id="editCategoryTag">
                    <div class="row merch_out" id="tag-image-content">
                            ...
                            <div class="col-md-3">
                                ...
                                @if($malltype->image)
                                    <div class="col-md-12 mb-3 pr-0">
                                        <img class="card-img-top fit-image" src="{{ $live_url.$malltype->image}}" alt="image count">
                                        <a  href="javascript:;" data-href="{{route('malltype.deleteimage',['id'=>$malltype->mt_id])}}" data-method="POST" class="btn-pi-delete" data-id="{{$malltype->mt_id}}">
                            ...
                            <div class="col-md-4">
                                <input type="text" name="type_name" placeholder="Enter type name" id="sub_category_name"
                                       class="form-control" required="" list="datalist1" value="{{ $malltype->type_name }}">
                            </div>
        ...
        $( function() {
            ...
            $(document).on('click','.upload-result', function (ev) {
                $uploadCrop.croppie('result', {
                    ...
                    fd.append("mt_id", "{{@$malltype->mt_id}}");

                    // console.log(fd);
                    $.ajax({
                        url: "{{route('malltype.uploadimage')}}",
