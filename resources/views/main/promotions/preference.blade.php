        ...
        <div class="col-md-10">
            <div class="card card-malle">
                ...
                <div class="card-body">
                                    ...
                                    <select id="preference_select">
                                        @if(!empty($preference_master_lists))
                                            @foreach($preference_master_lists as $preference_master)
                                                <option
                                                    value="{{ $preference_master->preference_id }}">{{ $preference_master->preference_name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <button type="submit" class="btn btn-primary col-md-12 top-t"
                                            id="btnMerchantPromotion">{{__('Add Preference')}}</button>
                                </div>
                            </div>
                    ...
                    <div class="row ">
                        <div class="col-md-12">
                            <table class="table table-striped malle-table " id="promotion-preference-table"
                                    ...
                                    <tr class="row-promo-tags" data-id="{{$preference_list->pp_id}}">
                                        ...
                                        <td>
                                            <select name="primary_tag" class="deal-status primary_tag"
                                                    data-id="{{$preference_list->pp_id}}"
                                                    data-href="{{route('promotion.preference.setprimary',['id'=>$preference_list->pp_id])}}"
                                                    data-method="POST">
                                                <option value="N"
                                                        @if($preference_list->primary_tag=="N") selected @endif>No
                                                </option>
                                                <option value="Y"
                                                        @if($preference_list->primary_tag=="Y") selected @endif>Yes
                                                </option>
                                            </select>
                                        </td>
                                        <td>
                                            <a href="javascript:;"
                                               data-href="{{route('promotion.preference.destroy',['promotions'=>$preference_list->pp_id])}}"
