        ...
        <div class="col-md-10">
            <div class="card card-malle">
                <div class="card-header-malle">
                   <p>Category Id: <span style="margin-right: 120px;color: red">{{ $categorys->Category_id }}</span> | Created On: <span style="margin-right: 120px;color: red">{{ $categorys->Created_on }}</span> | Created By: <span style="color: red">{{ \App\User::getUserName( $categorys->Created_by)  }}</span> <span style="float: right;color: blue"><a href="{{ route('category.header') }}">Back</a></span></p>
                </div>
                <div class="card-body" id="tag-image-body" data-sourceurl="{{route('category-tags.show',[$categorys->Category_id])}}">
                    <form method="PATCH" action="{{route('category.header.update',[$categorys->Category_id])}}" id="editCategoryTag">
                    <div class="row merch_out" id="tag-image-content">
                            ...
                            <div class="col-md-3">
                                ...
                                @if($categorys->image)
                                    <div class="col-md-12 mb-3 pr-0">
                                        <img class="card-img-top fit-image" src="{{ $live_url.$categorys->image}}" alt="image count">
                                        <a  href="javascript:;" data-href="{{route('category.header.deleteimage',['id'=>$categorys->Category_id])}}" data-method="POST" class="btn-pi-delete" data-id="{{$categorys->Category_id}}">
                            ...
                            <div class="col-md-3">
                                <input type="text" name="Category_name" placeholder="Enter Category Tag" id="category_name"
                                       class="form-control" required="" list="datalist1" data-autocompleturl="{{route('category.header.search')}}" value="{{ $categorys->Category_name }}">
                            </div>
        ...
        $( function() {
            ...
            $(document).on('click','.upload-result', function (ev) {
                $uploadCrop.croppie('result', {
                    ...
                    fd.append("Category_id", "{{@$categorys->Category_id}}");

                    // console.log(fd);
                    $.ajax({
                        url: "{{route('category.header.uploadimage')}}",
